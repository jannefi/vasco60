...
def _run_tskymatch2(stilts: str, in_csv: Path, vsx_fits: Path, out_csv: Path, radius_arcsec: float) -> None:
    subprocess.run(
        [
            ...
            # (vasco/cli_pipeline.py:1307). With find=best, two candidates
            # within 5" of one VSX star only get one match — the farther
            # candidate slips through as an unmatched false survivor.
            "find=best1",
            f"out={str(out_csv)}",
            "ofmt=csv",
        ],
